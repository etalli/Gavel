...
言い換えると、「Claude Code の提案を盲目的に受け入れるな。立ち止まれ。考えろ。そして決断せよ。」
Gavel は、オートパイロットモードへの物理的なリマインダーであり、歯止めです。
Happy coding!
...
- **ログ** — ボタンを押すたびに `~/.claude/gavel/decisions.jsonl` へ記録が追記されます。`python3 hooks/stats.py` で集計を確認できます。
